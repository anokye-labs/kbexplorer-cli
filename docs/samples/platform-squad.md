...
This file is a small, human-readable sample source for `kbx derive`. It
is intentionally prose: `derive` reads `.docx`, prose Markdown, and plain text
identically. Running

```bash
kbx derive docs/samples/platform-squad.md
```
...
output and `kbx derive docs/samples/platform-squad.md --check` stays green
