...
| [Platform & Template Author Guide](./platform-guide.md) | Teams **standardizing kbx across an org** | Install modes, custom templates + the template contract, the source record, fleet-wide updates, cross-repo reuse, roadmap | ~30 min |
...
graph TB
    ...
    subgraph host["Host repo — what the CLI writes/reads"]
        kbdir[".kbx/ (explorer app)"]:::store
        rec[".kbx.json (source record)"]:::store
        env[".env.kbx"]:::store
    ...
    init --> host
    gen --> content
    dev --> viteapp["Vite dev server<br>(from .kbx/)"]:::ext
...
- **Host repo** — the repository you run `kbx init` in. The CLI installs into it.
- **Template / explorer app** — the Vite web app that renders the graph, installed at `.kbx/`.
...
- **Source record** — `.kbx.json`, the CLI-owned record of where the template came from.
