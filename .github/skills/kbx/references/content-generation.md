...
How kbx generates knowledge base content from repository analysis, using
the kb-architect and kb-writer agents.

## kbx Frontmatter Format
...
script converts it into kbx content:

| Catalogue Field | kbx Field | How It Maps |
|----------------|-----------------|-------------|
...
registered components in kbx's `FLUENT_ICONS` registry (`src/components/NodeVisual.tsx`):
