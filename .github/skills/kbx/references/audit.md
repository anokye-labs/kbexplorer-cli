...
`kbx audit` is the hard-error linter for content. It complements
`kbx links` (soft graph-health analysis) by catching the issues that
...
kbx audit --json           # machine-readable JSON
kbx audit --content docs   # override content directory
```
...
**Fix**: either create the parent (`kbx scaffold <parent-id> --cluster …`)
...
| When | What |
|---|---|
| Before every commit touching `content/` | `kbx audit` |
| In CI on every PR | `kbx audit` (gates merge) |
| After bulk operations (rename, merge, split) | `kbx audit && kbx links` |
| Weekly housekeeping | both, plus `kbx affected origin/main` |

## What audit does NOT cover

| Concern | Where to find it |
|---|---|
| Orphan nodes (no edges) | `kbx links` |
| Weak clusters (no cross-cluster edges) | `kbx links` |
| Coverage gaps (source files no node cites) | `kbx links` |
| Unlinkified mentions | `kbx links` |
| Drift between code and content | `kbx affected <ref>` + `update-node.md` |
