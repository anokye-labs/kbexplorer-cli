...
`incremental-refresh.md` (which uses `kbx affected` to find the
...
A fast way to find drifted citations: run `kbx affected <ref>` first,
...
kbx links                          # check no new warnings introduced
```

Then preview the page in the explorer (`npx kbx dev`, navigate to the
...
other nodes that pointed to it — run `kbx audit` afterwards to catch
the ones you missed.
