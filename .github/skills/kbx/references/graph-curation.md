...
Always run `kbx audit && kbx links` before AND after any
...
4. Run `kbx audit` — there should be no new warnings.
...
(`kbx scaffold`), then update children.
...
2. `kbx scaffold` each new node (set the original node as `parent:` if
   appropriate).
...
`kbx links` reports orphan nodes — nodes with zero edges. For each:
