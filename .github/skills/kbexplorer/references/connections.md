...
`kbexplorer links` flags `redundantFrontmatter` when both forms reference
...
```bash
kbexplorer audit            # catches dead connections (typos, deleted nodes)
kbexplorer links            # surfaces weak clusters (no cross-cluster edges)
