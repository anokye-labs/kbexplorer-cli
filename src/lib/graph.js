/**
 * Knowledge-graph loader and graph-native utilities for the `kbexplorer mcp` server.
 *
 * Builds an in-memory graph from authored `content/*.md` nodes (id / title /
 * cluster / parent / emoji / connections), confined to a set of granted root
 * directories. The MCP server scopes everything it shares with the host model
 * to these roots — the client's `roots` capability decides what the server may
 * read, and this module enforces that boundary.
 *
 * Context assembly for `kb_ask` is explicit and graph-native: the host model
 * navigates the graph via `kb_get_node`, `kb_neighbors`, and `kb_graph_stats`,
 * then supplies specific node ids to `kb_ask` for grounded sampling. No lexical
 * keyword scoring is performed here.
 *
  ...
  for (const dir of scanDirs) {
    for (const file of listMarkdownFiles(dir)) {
      ...
      nodes.set(fm.id, {
        ...
        body: parsed.body ?? '',
        path: file,
        relPath: relative(effectiveRoots[0] ?? resolve(cwd), file).split(sep).join('/'),
      });
    }
  }
