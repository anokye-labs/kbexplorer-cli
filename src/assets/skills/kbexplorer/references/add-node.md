...
Use this workflow instead of `kbexplorer generate` whenever the rest of the
...
| **cluster** | MUST exist in `content/config.yaml`. Run `kbexplorer audit` first to see declared clusters; create a new one only if no existing cluster fits — see `graph-curation.md`. |
...
```bash
kbexplorer scaffold <slug> --cluster <id> [--parent <id>] [--title "..."] [--emoji "..."]
```
...
kbexplorer scaffold worker-pool --cluster runtime

# Full
kbexplorer scaffold auth-flow \
  --cluster security \
  --parent core \
...
HTML comment, and a placeholder body. The file passes `kbexplorer audit`
immediately.
...
```bash
kbexplorer audit          # hard structural errors — must be 0
kbexplorer links          # soft graph health — investigate any new warnings
```
