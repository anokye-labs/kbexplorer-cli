...
1. Scaffold the node (`kbexplorer scaffold ...`).
2. Drop the image into `content/assets/heroes/<slug>.jpg` (or sprite).
3. Add the `image:` / `sprite:` line to the frontmatter.
4. Run `kbexplorer audit` (frontmatter is still valid even if the image is
...
3. Use `kbexplorer links` to find the most connected nodes and start there.
...
A future `kbexplorer audit --check-assets` flag could automate this. For
