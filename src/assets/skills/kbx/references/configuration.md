...
kbx's behavior, appearance, and content.
...
These are typically set in `.env.kbx` by the init script.

## Runtime Block (`.kbx.json`)

The optional `runtime` block in `.kbx.json` sets a repo-local default
...
|----------|--------|---------|
| 1 | `--runtime <name>` CLI flag | `kbx derive --runtime claude` |
| 2 | `runtime` block in `.kbx.json` | (this section) |
| 3 | `KBX_RUNTIME` env var | `KBX_RUNTIME=claude …` |
| 4 | Default | `copilot` |
...
| Env var | Adapter |
|---------|---------|
| `KBX_COPILOT_BIN` | `copilot` |
| `KBX_CLAUDE_BIN` | `claude` |
...
kbx doctor --runtime claude  # check a specific adapter
kbx doctor --json            # machine-readable output for tooling
kbx doctor --offline         # skip the latest-tag network call
```
