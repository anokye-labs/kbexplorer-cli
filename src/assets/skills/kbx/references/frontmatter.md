...
> **Schema scope.** The kbx CLI parser handles a deliberate
...
Anything else triggers a `dead-connection` warning in `kbx audit`.
...
exist, both recognised by `kbx affected`:
...
| Symptom | Cause | Fix |
|---|---|---|
| Page does not appear in the graph | Missing `id` or `cluster` | Add the required field; re-run `kbx audit`. |
