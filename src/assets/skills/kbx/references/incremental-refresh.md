...
`kbx affected` to map a code diff to the impacted nodes, then drives
...
- **The change is documentation-worthy** → use `add-node.md` to create a new
  page covering it. After scaffolding and writing, re-run `kbx affected`
...
```bash
kbx audit          # hard errors must be 0
kbx links          # check for new orphans / weak clusters
```
...
- run: npx kbx audit                       # fails the job on errors
- run: npx kbx affected origin/main --json > affected.json
- run: |
